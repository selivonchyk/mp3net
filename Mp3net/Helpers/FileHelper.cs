	...
	class FileHelper
	{
		...
		static FileHelper ()
		{
			if (Environment.OSVersion.Platform.ToString ().StartsWith ("Win"))
				...
				var ufh = Type.GetType("Mp3net.Unix.UnixFileHelper, Mp3net.Unix");
				if (ufh == null) {
					var path = ((FilePath) typeof (FileHelper).Assembly.Location).GetParent();
					var assembly = Assembly.LoadFile(Path.Combine(path, "Mp3net.Unix.dll"));
					if (assembly == null)
						throw new Exception("Mp3net.Unix.dll is required when running on a Unix based system");
					ufh = assembly.GetType("Mp3net.Unix.UnixFileHelper");
				}
